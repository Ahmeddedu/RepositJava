...
// Класс для демонстрации работы с коллекциями
public class Les {

    public static void main(String... args) {
        if (args.length > 0) {
            handleSetOperations(); // Проверяем, переданы ли аргументы и, если да, выполняем операции с множествами
        } else {
            // Работа с LinkedList и ArrayList
            handleListOperations();
        }
    }

    private static void handleListOperations() {
        // Работа с LinkedList
    ...
    private static void handleSetOperations() {
        // Работа с множеством
        ...
        System.out.println("Removing non-existing element: " + s.remove(3)); // Пытаемся удалить несуществующий элемент
        System.out.println("IsEmpty: " + s.isEmpty()); // Проверяем, пусто ли множество

        // Работа с дополнительной коллекцией
    ...
    private static void printIterable(Iterable<?> iterable) {
        ...
        for (Object item : iterable) {
            System.out.println(item); // Печатаем каждый элемент коллекции
        }
